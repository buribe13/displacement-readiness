...
Generated by Outreach Window Planner (simulated data)
  ...
  // Create alert from overlap
  const createAlertFromOverlap = (overlap: SignalOverlap) => {
    if (!onCreateAlert) return;

    const overlapSignals = getOverlapSignals(overlap);
    const suggestion = generateCoordinationSuggestion(overlap, overlapSignals);

    onCreateAlert({
      title: `Overlap: ${overlap.combinedImpact} impact`,
      notes: `${overlap.whyItMatters}\n\nCoordination suggestion:\n${suggestion}`,
      timeRange: overlap.timeRange,
      source: "overlap",
      relatedIds: {
        overlapId: overlap.id,
        signalIds: overlap.signalIds,
      },
    });
  };
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              size="icon"
                              className="h-8 w-8"
                              onClick={() => createAlertFromOverlap(overlap)}
                            >
