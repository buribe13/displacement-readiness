...
import type { CivicSignal, SignalKind, PressureLevel } from '@/lib/signals/types';
import {
  KOREATOWN_CENTER,
 ...
 */
const PRESSURE_COLORS: Record<PressureLevel, { bg: string; text: string; label: string }> = {
  low: { bg: 'bg-[oklch(0.95_0.02_145)]', text: 'text-[oklch(0.35_0.05_145)]', label: 'Low' },
  elevated: { bg: 'bg-[oklch(0.93_0.03_75)]', text: 'text-[oklch(0.35_0.08_75)]', label: 'Elevated' },
  high: { bg: 'bg-[oklch(0.92_0.04_35)]', text: 'text-[oklch(0.35_0.1_35)]', label: 'High' },
};
...
export function MapPanel({ signals, pressureLevel = 'low', onSignalClick }: MapPanelProps) {
  const mapContainer = useRef<HTMLDivElement>(null);
  const map = useRef<mapboxgl.Map | null>(null);
  const [mapLoaded, setMapLoaded] = useState(false);
  const [layerVisibility, setLayerVisibility] = useState<Record<SignalKind, boolean>>({
    sanitation: true,
    eventPermit: true,
    roadClosure: true,
        ...
        new mapboxgl.Popup({ closeOnClick: true, maxWidth: '300px' })
          ...
          .setHTML(`
            <div style="font-family: system-ui; font-size: 13px;">
              <div style="font-weight: 600; margin-bottom: 4px;">${props?.description || 'Signal'}</div>
              <div style="color: #666; font-size: 11px; margin-bottom: 8px;">
                Source: ${props?.source || 'Unknown'}
                ${props?.isSimulated ? '<span style="background: #f0f0f0; padding: 1px 4px; border-radius: 3px; margin-left: 4px;">Simulated</span>' : ''}
              </div>
              <div style="font-size: 12px; color: #444;">${props?.interpretation || ''}</div>
              <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #eee; font-size: 10px; color: #888;">
                Confidence: ${props?.confidence || 'unknown'} · Data is delayed/simulated
              </div>
            </div>
          `)
          .addTo(map.current!);
  ...
  useEffect(() => {
    ...
    Object.entries(layerVisibility).forEach(([kind, visible]) => {
      ...
      if (map.current!.getLayer(`${layerId}-line`)) {
        map.current!.setLayoutProperty(`${layerId}-line`, 'visibility', visibility);
      }
      if (map.current!.getLayer(`${layerId}-fill`)) {
    ...
    <Card className="h-full flex flex-col">
      <CardHeader className="py-3 px-4 flex-none">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-medium">Koreatown Map View</CardTitle>
      ...
      <CardContent className="flex-1 p-0 relative">
        ...
        <div className="absolute bottom-4 left-4 bg-background/95 border rounded-md p-3 text-xs space-y-3 max-w-[200px]">
              ...
              {(Object.keys(SIGNAL_LABELS) as SignalKind[]).map((kind) => (
                <div key={kind} className="flex items-center gap-2">
                  <div
                    ...
                    style={{ backgroundColor: SIGNAL_COLORS[kind] }}
                  />
                  <span className="text-muted-foreground">{SIGNAL_LABELS[kind]}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1">
              {(Object.keys(PRESSURE_COLORS) as PressureLevel[]).map((level) => (
                <div key={level} className="flex items-center gap-2">
                  <div className={`w-2.5 h-2.5 rounded ${PRESSURE_COLORS[level].bg}`} />
                  <span className="text-muted-foreground">{PRESSURE_COLORS[level].label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="text-muted-foreground">
            All data is simulated or delayed (24h+). Not for real-time decisions.
          </div>
        </div>
        
        {/* Current Pressure Indicator */}
        <div className="absolute top-4 right-4">
          <div className={`${PRESSURE_COLORS[pressureLevel].bg} ${PRESSURE_COLORS[pressureLevel].text} px-3 py-1.5 rounded-md text-xs font-medium`}>
